...
function onMessage(room, client, type, data, stageIndex) {
  ...
  if (type === "locationGuess" || type === "location") {
    ...
    if (isLocationCorrect(text)) {
      ...
      room.addToHistory(stageIndex, { locationCorrect: true, zoomLevel: payload.zoomLevel });
  ...
  if (payload.phase === "result" && (type === "next" || type === "continue")) {
    ...
    if (nextSub >= MAX_SUB_ROUNDS) {
      room.addToHistory(stageIndex, { zoomLevel: payload.zoomLevel, subRounds: MAX_SUB_ROUNDS });
      room.advanceToInterim(7);
      return true;
    }
