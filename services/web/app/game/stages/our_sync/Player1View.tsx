...
export function Player1View({ state, room }: Props) {
  ...
  const myAllowed = isSwapped ? PLAYER_2_ALLOWED : PLAYER_1_ALLOWED;
  const successMessage = state.player1Text || COPY.successMessage;
  const currentNumber = payload.remainingSteps ?? 0;
  const currentDirectionLabel = directionDisplayLabel(payload.currentDirection);
  const infoValue = !isSwapped ? String(currentNumber) : currentDirectionLabel;
