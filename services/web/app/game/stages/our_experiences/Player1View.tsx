...
const PLAYER_A_PATH = ["LEFT", "DOWN", "RIGHT", "DOWN", "LEFT"];

/** Phones: show only the fun fact text; no timer (TV has the timer and sends funFactEnd). */
export function FunFactCountdownView({ room }: { room: { send: (type: string) => void } }) {
  const theme = useGameTheme();
  return (
    <div dir="rtl" style={{ textAlign: "center", padding: 24 }}>
      <h2 style={{ fontSize: theme.typography.phoneTitle, color: theme.colors.accent, margin: "0 0 12px" }}>
        {COPY.funFactTitle}
      </h2>
      <p style={{ fontSize: theme.typography.phoneCaption, color: theme.colors.textMuted, whiteSpace: "pre-line", margin: "0 0 24px" }}>
        {COPY.funFactBody}
      </p>
      <p style={{ fontSize: theme.typography.phoneCaption, color: theme.colors.textMuted, margin: 0 }}>
        {COPY.funFactCta}
      </p>
    </div>
  );
...
export function Player1View({ state, room, questionnaire }: Props) {
  ...
  const steps = payload.playerAStepHistory || [];
  ...
  const iAnswered = payload.player1AnswerCurrent != null;
  const partnerAnswered = payload.player2AnswerCurrent != null;
  const waitingForPartner = iAnswered && !partnerAnswered;

  if (phase === "guessNumber" || (phase === "readyToGuess" && showDigitScreenLocal)) {
    const feedback = state.player1Text || "";
    const isCorrect = payload.playerACorrect === true;
